...
FUNCIONES PARA EL JUEGO DE CARAMELOS O DULCES

*/

//Gemerar números aleatorios
...
// Validar si se deben eliminar por columna
function Validar_Colum() {
	for (var j = 0; j < 7; j++) {
		var counter = 0;
		var candyPosition = [];
		var extraCandyPosition = [];
		var candyColumn = arreglo_column(j);
		...
		if (candyCount >= 3) {
			Eliminar_column(candyPosition, candyColumn);
			Calcular_puntuacion(candyCount);
		}
	}
}

// Crear dulces
function Obtener_dulces(arrayType, index) {
...
function Eliminar_column(candyPosition, candyColumn) {
...
function arreglo_fila(index) {
	var candyRow = Obtener_dulces('rows', index);
	return candyRow;
}

// arreglos de colunmnas
function arreglo_column(index) {
	var candyColumn = Obtener_dulces('columns');
	return candyColumn[index];
}


function Eliminar_hor(candyPosition, candyRow) {
	for (var i = 0; i < candyPosition.length; i++) {
...
function Calcular_puntuacion(candyCount) {
...
function Llenar_tablero() {
	...
	column.each(function () {
		...
		for (var i = 0; i < agrega; i++) {
			var candyType = Generar_aleatorios(1, 5);
			if (i === 0 && candys < 1) {
...
// Validar si se deben eliminar por fila
function Validar_Fila() {
	for (var j = 0; j < 6; j++) {
		var counter = 0;
		var candyPosition = [];
		var extraCandyPosition = [];
		var candyRow = arreglo_fila(j);
		...
		candyCount = candyPosition.length;
		if (candyCount >= 3) {
			Eliminar_hor(candyPosition, candyRow);
			Calcular_puntuacion(candyCount);
		}
	}
}
...
//Movimiento de los dulces
function Mover_dulces() {
	$('img').draggable({
...
//Reemplazar los dulces
function Reemplazar(event, candyDrag) {
